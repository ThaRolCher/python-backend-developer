...
async def test_controller_create_duplicate_should_return_bad_request(client, products_url):
    await client.post(products_url, json=product_data())
    response = await client.post(products_url, json=product_data())
    
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert response.json()["detail"] == "Product already exists with name: Iphone 14 Pro Max"
...
@pytest.mark.usefixtures("products_inserted")
async def test_controller_query_by_price_range_should_return_correct_items(client, products_url):
